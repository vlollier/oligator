...
class SmilesEncoder:  
    ...
    # les branches quoi...
    def jejardine(self,inode,lsnode):    
        """
        ...
        elif "anh" in self.G.nodes[inode]:
            print((len(self.cycles),self.G.nodes[inode]["anh"]))
            smi+=str(len(self.cycles)+self.G.nodes[inode]["anh"])
        
        
        if len(neigh)>0:
            if self.in_cycle(inode):
                # start cycle path
                if self.end==None and not is_end:
                    cycle_neigh=[]
                    atom_neigh=[]
                    for n in neigh:
                        if self.in_cycle(n):
                    ...
                    if 'O' in atom_neigh:
                        self.end=cycle_neigh[atom_neigh.index('O')]
                    else:
                        self.end=max(cycle_neigh) 
                          
                    neigh.remove(self.end)
                    smi+=str(self.cnum)
            
            
            for n in neigh[:-1]:                
                smi+="("+self.jejardine(n,lsnode)+")"
            smi+=self.jejardine(neigh[-1:][0],lsnode)
    ...
    def graph2smi(self):
        ...
        return self.jejardine(self.redend,[])
            ...
            for j in range(i+1,len(self.cycle_nodes)):                
                ...
                if len(inter)!=0:
                    ...
                    if len(s1)<len(s2):
                        ...
                        ose=list(self.cycle_nodes[j])
                    else:
                        bond=list(self.cycle_nodes[j])
                        ose=list(self.cycle_nodes[i])
                    for node in bond:
                    ...
                    if onode>0:
                        ...
                        for node in neigh:
                            if re.sub("[0-9]+","",self.Gatom.nodes[node]["atom"])=="C" and node in ose:
                                cnode=node
                                break
        ...
        if len(branchages)>0:
            ...
            for n in lsnum:
                ...
                for i in range(0,len(p),2):
                    ...
                    Logger.debug("Cycle:%s %i - %s %i"%(self.nodatoms[ideb],ideb,self.nodatoms[ifin],ifin))
                ...
                for node in g.nodes():
                    if len(g.nodes[node])>0:
                        if g.nodes[node]["cnum"]==cnum:
                            if g.nodes[node]["atom"]==ose.carbons[ose.mods.index(alin)]  :
                                connector=node
...
class ose_template:    
    ...
    def get_graph_cnum(self,gatom):
        ...
        if len(subcyc)>0:        
            ...
            dico_subcx={}
            for sub in subcyc:
                path=nx.shortest_path(gprime,sub[0],ose[0])
                link=list(set(path) - set(sub))
                link=list(set(link) - set(ose))
                print(link)
                for i in range(0,len(path)-1):  
                    if path[i] in sub and path[i+1] in link:                          
                        e=(path[i],path[i+1])
                        print(e)
                        gprime.remove_edge(*e)
                                           
                        toremove=__collect4__(gprime,path[i],path[i],[path[i]])
    ...
    def graph_cnum(self,gatom,cycle):        
        ...
        for node in gatom.nodes():
            ...
            if node in cycle:
                ...
                if re.sub("[0-9]+","",gatom.node[node]["atom"])=="O":
                    oxnode=node
            else:
                    ...
                    if substit!=None:
                        ...
                        for neigh in self.Gatom[atomnode]:                       
                            ...
                            if neigh in substit.nodes():
                                clinks.append([subformul,Topograph.__simple__(substit,neigh,-1)]) 
                    else:
                        anh=False
                        # anhydro?
                        for edge in self.anhydro_edge:
